...
const BUILTIN_PERSONAS: PersonaDefinition[] = [productOwner, deliveryManager, techLead];

export function getPersona(idOrShortName: string): PersonaDefinition | undefined {
  const key = idOrShortName.toLowerCase();
  return BUILTIN_PERSONAS.find((p) => p.id === key || p.shortName === key);
}
...
export function resolvePersonaId(input: string): string {
  ...
  if (!persona) {
    const available = BUILTIN_PERSONAS.map((p) => `${p.shortName} (${p.name})`).join(", ");
