...
export function tlHealthPage(ctx: PersonaPageContext): string {
  ...
  const techTrending = upcoming.trending.filter((t) =>
    ["task", "action"].includes(t.type),
  );
  ...
  const processSection = collapsibleSection(
    ...
    `<div class="gar-areas">
      ...
      <div class="gar-area">
        ...
        ${cat.items.length > 0
          ? `<ul>${cat.items.slice(0, 5).map((item) => `<li><span class="ref-id">${escapeHtml(item.id)}</span>${escapeHtml(item.detail)}</li>`).join("")}</ul>`
          : ""}
      </div>`).join("")}
    </div>`,
    { titleTag: "h3" },
  ...
  // Trending technical items
  const trendingSection = techTrending.length > 0
    ? collapsibleSection(
        "tl-health-trending",
        "Trending Technical Items",
        `<div class="table-wrap">
